...
class BarcodeScannerService: NSObject, ObservableObject {
    ...
    private func handleDetectedBarcodes(request: VNRequest, error: Error?) {
        ...
        guard let bestBarcode = validBarcodes.max(by: { $0.confidence < $1.confidence }) else {
            return
        }
        
        // Enhanced validation - only proceed with high-confidence detections
        let minimumConfidence: Float = bestBarcode.barcodeType == .qr ? 0.6 : 0.8
        guard bestBarcode.confidence >= minimumConfidence else {
            print("🔍 Barcode confidence too low: \(bestBarcode.confidence) < \(minimumConfidence)")
            return
        }
        
        // Ensure barcode string is valid and not empty
        guard !bestBarcode.barcodeString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            print("🔍 Empty or whitespace-only barcode string detected")
            return
        }
        
        // Check for duplicates
        guard !recentlyScannedBarcodes.contains(bestBarcode.barcodeString) else {
            print("🔍 Skipping duplicate barcode: \(bestBarcode.barcodeString)")
            return
        }
        ...
        print("🔍 ✅ Valid barcode detected: \(bestBarcode.barcodeString) (confidence: \(bestBarcode.confidence), minimum: \(minimumConfidence))")
        
        // Add to recent scans to prevent duplicates
        recentlyScannedBarcodes.insert(bestBarcode.barcodeString)
        
        // Publish result on main queue
        DispatchQueue.main.async { [weak self] in
            self?.lastScanResult = bestBarcode
            ...
            os_log("Barcode detected: %{public}@ (confidence: %.2f)", 
                   ...
                   type: .info, 
                   bestBarcode.barcodeString, 
                   bestBarcode.confidence)
        }
    }
